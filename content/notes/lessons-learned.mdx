...
---
  ...
  The first version just minted a JWT and called it done. A real login
  needs lockout on repeated failures, 2FA at the API layer (not just the
  UI), and security-stamp revocation so a password change actually kills
  existing sessions. Each of these is small on its own; skipping them
  quietly leaves the door open.
  ...
  Shipping v1 meant naming these out loud rather than pretending they
  didn't exist.
